...
$Thread_Name = mysql_real_escape_string($Thread_Name);
$BoardID = $_SESSION["board_id"];
$UserID = $_SESSION["user_id"];
$ThreadDes = $_GET["description"];
$ThreadDes = mysql_real_escape_string($ThreadDes);
$Int = rand(0, 10000000);
$sql = "INSERT INTO Threads (idThreads, Thread_Name, Thread_Creation_Date, User_idUser, Board_idBoard, Thread_Archived, Thread_Description, Bump_Date)
VALUES (FLOOR('$Int'), '".$conn->real_escape_string($_GET["Threadname"])."', NOW(), '$UserID','$BoardID', 'f', '".$conn->real_escape_string($_GET["description"])."', NOW())";
if ($conn->query($sql) === TRUE) {
} else {
    echo "Error: " . $sql . "<br>" . $conn->error;
}
$sql = "UPDATE Threads
SET Thread_Description = REPLACE(Thread_Description, CHAR(10), ' </br> ')
WHERE idThreads = FLOOR($Int)";
if ($conn->query($sql) === TRUE) {
} else {
    echo "Error: " . $sql . "<br>" . $conn->error;
}
	...
	<body>
		<div id="wrapper">
			...
			<main>
				<div id="content">
					<div class="innertube">
						<h1>Your thread has been posted!</h1>
						<?php
                        if($_SESSION['board_id'] == 1){
						   echo '<li><a href="/~junjie2412/NewsBoard.php">Back to News</a></li>';
                        }
